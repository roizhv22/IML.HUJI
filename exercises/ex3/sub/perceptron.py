...
class Perceptron(BaseEstimator):
    ...
    def __init__(self,
        ...
        """
        super().__init__()
        self._iterating = False  # protection for multiple intercept addition
        # in predict function during fitting to be able manage the callback.
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        """
        Fit a halfspace to to given samples.
        ...
        self.fitted_, self._iterating = True, True
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        if self.include_intercept_ and not self._iterating:
            X = np.insert(X, 0, 1, axis=1)

        ret = np.zeros((len(X),))
        for i in range(len(X)):
            ret[i] = np.sign(np.dot(self.coefs_, X[i]))
        return ret
