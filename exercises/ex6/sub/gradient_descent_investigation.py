...
def get_gd_state_recorder_callback() -> Tuple[
    ...
    def callback(value, weights, jacobian, t, eta):
...
def compare_fixed_learning_rates(
    ...
    for j, model in enumerate([L1, L2]):
        ...
        for i, eta in enumerate(etas):
            ...
            cov_plot.update_layout(
                title=f"{model.__name__} with eta={eta}, min achieved is "
                      f"{np.min(values_l1)}",
...
def compare_exponential_decay_rates(
    ...
    conv_fig.update_layout(title="Q5", xaxis_title="GD iteration",
                           yaxis_title="norm value")
